...
char *const where_name[] = {
  ...
  "&w<standing on>         ",
  "&w<worn on tail>        ",
  "&w<floating above head> ",
  "&w<worn as scouter>     ",
  "&w<Error Inform Coder>  ",
  "&w<Error Inform Coder>  ",
  "&w<Error Inform Coder>  ",
  "&w<Error Inform Coder>  ",
  "&w<Error Inform Coder>  ",
...
void show_visible_affects_to_char( CHAR_DATA * victim, CHAR_DATA * ch )
{
  ...
  if( IS_AFFECTED( victim, AFF_LSSJ ) )
  {
    ...
    if( !is_leet( ch ) )
    {
      act( AT_YELLOW, "$N's hair is wildly spiked and eyes bleached white.", ch, NULL, victim, TO_CHAR );
      pager_printf_color( ch, "  &P*" );
      act( AT_YELLOW, "$N has a huge golden aura.&D", ch, NULL, victim, TO_CHAR );
      if( IS_AFFECTED( victim, AFF_MAJIN ) )
      {
        pager_printf_color( ch, "  &P*" );
        act( AT_RED, "$N has bolts of evil transacentdant power flowing through $s aura.&D", ch, NULL, victim, TO_CHAR );
      }
      else
      {
        pager_printf_color( ch, "  &P*" );
        act( AT_BLUE, "$N has bolts of transacentdant power flowing through $s aura.&D", ch, NULL, victim, TO_CHAR );
      }
    }
    else
    {
...
void show_char_to_char_0( CHAR_DATA * victim, CHAR_DATA * ch )
{
  ...
  if( is_saiyan( victim ) )
  {
    ...
    {
      case 1:
        strcat( buf, "Elite Saiyan " );
        break;
      case 2:
        if( victim->sex == SEX_MALE )
          strcat( buf, "Saiyan Prince " );
        else if( victim->sex == SEX_FEMALE )
          strcat( buf, "Saiyan Princess " );
        else
          strcat( buf, "Low Royal Saiyan " );
        break;
      case 3:
        if( victim->sex == SEX_MALE )
          strcat( buf, "Saiyan King " );
        else if( victim->sex == SEX_FEMALE )
          strcat( buf, "Saiyan Queen " );
        else
          strcat( buf, "High Royal Saiyan " );
        break;
...
//      sprintf( plbuf, " &G(&w%s&G)", num_punct_ld( victim->pl ) );
         ...
         !wearing_sentient_chip( ch ) && ( scouter = has_scouter( ch )) == NULL )
	{
      ...
      else if( !is_android( ch ) && !is_superandroid( ch ) && !wearing_sentient_chip( ch ) && ( scouter = has_scouter( ch ) ) )
      {
      ...
      else if( is_android( ch ) || is_superandroid( ch ) || wearing_sentient_chip( ch ) )
      {
        int a = 0;
    ...
    case POS_STANDING:
      if( IS_IMMORTAL( victim ) )
        ...
        if( ( victim->in_room->sector_type == SECT_UNDERWATER )
            ...
            && !is_icer( victim )
            && !is_android( victim ) && !is_bio( victim ) && !is_superandroid( victim ) && !is_genie( victim ) )
...
void do_look( CHAR_DATA * ch, char *argument )
{
  ...
  if( !IS_NPC( ch )
      ...
      && room_is_dark( ch->in_room ) && !is_android( ch ) && !is_superandroid( ch ) && !is_demon( ch ) )
  {
...
void do_who( CHAR_DATA * ch, char *argument )
{
       ...
       d = ( fGroup ? ( whogr ? whogr->d : NULL ) : d->prev ) )
  {
    ...
    if( is_kaio( wch ) && wch->rank > 0 )
    {
      kaioshin = TRUE;
      sprintf( rank, "%s", get_kai( wch ) );
    }
    else if( is_demon( wch ) && wch->rank > 0 )
    {
      kaioshin = TRUE;
      sprintf( rank, "%s", get_demon( wch ) );
    }
    else if( is_saiyan( wch ) && wch->rank > 0 )
    {
      kaioshin = TRUE;
      sprintf( rank, "%s", get_saiyan( wch ) );
    }
    else
      kaioshin = FALSE;
...
*/
    sprintf( class_text, " %s%15s", get_pkColor( wch ), kaioshin ? rank : class_table[wch->class]->who_name );
    ...
    {
      sprintf( buf2, " &W%20s", wch->pcdata->pretitle );
      class = buf2;
    }
...
*/
  pager_printf_color( ch,
                      "&z     &Y%d &Wplayer%s.&z      -       &Wwww.dbnewage.com       &z-&W       Max: &B%d      &z\n\r",
  ...
  if( arg[0] == '\0' )
  {
    ...
    for( d = first_descriptor; d; d = d->next )
      if( ( d->connected == CON_PLAYING || d->connected == CON_EDITING )
          ...
          && !saiyanranked( victim ) && !demonranked( victim ) && !kairanked( victim ) )
...
// This is the gtrain limit code. Unneeded. Gtrain should be fixed. 
// Yet, I'll leave it just in case.
//                              -Karn 06.18.05 
/*  if( ch->level >= 10 )
  {
    send_to_char( "You do not need to train anymore.\n\r", ch );
    return;
  }
*/
  /*
  ...
  if( ch->train < 1 )
  {
    send_to_char( "You don't have enough training points.\n\r", ch );
    return;
  }
  ...
  if( ( ch->perm_str >= 100 && ch->level < 10 ) || ( ch->perm_str >= 105 && ch->level >= 10 ) )
    ...
    {
      send_to_char( "You are already at your max strength.\n\r", ch );
      return;
    }
  if( ( ch->perm_dex >= 100 && ch->level < 10 ) || ( ch->perm_dex >= 105 && ch->level >= 10 ) )
    if( !str_cmp( argument, "spd" ) || !str_cmp( argument, "speed" ) )
    {
      send_to_char( "You are already at your max speed.\n\r", ch );
      return;
    }
  if( ( ch->perm_con >= 100 && ch->level < 10 ) || ( ch->perm_con >= 105 && ch->level >= 10 ) )
    if( !str_cmp( argument, "con" ) || !str_cmp( argument, "constitution" ) )
    {
      send_to_char( "You are already at your max constitution.\n\r", ch );
      return;
    }
  if( ( ch->perm_int >= 100 && ch->level < 10 ) || ( ch->perm_int >= 105 && ch->level >= 10 ) )
    if( !str_cmp( argument, "int" ) || !str_cmp( argument, "intelligence" ) )
    {
      send_to_char( "You are already at your max intelligence.\n\r", ch );
      return;
    }
  if( !str_cmp( argument, "str" ) || !str_cmp( argument, "strength" ) )
  {
  ...
  if( pOutput == NULL )
  {
    send_to_char( "you can only train int, spd, con, str!\n\r", ch );
    return;
  }
  ...
  *pAbility += 1;
  act( AT_RED, "You have your $T automatically upgraded!", ch, NULL, pOutput, TO_CHAR );
...
void do_practice( CHAR_DATA * ch, char *argument )
{
  ...
  if( ( !str_cmp( argument, "majin" ) || !str_prefix( argument, "majin" ) ) && argument[0] != '\0' )
  {
    ch_printf( ch, "You can't.\n\r" );
    return;
  }
  if( ( !str_cmp( argument, "t1" ) || !str_prefix( argument, "t1" ) ) && argument[0] != '\0' )
  {
    ch_printf( ch, "You can't.\n\r" );
    return;
  }
  if( ( !str_cmp( argument, "t2" ) || !str_prefix( argument, "t2" ) ) && argument[0] != '\0' )
  {
    ch_printf( ch, "You can't.\n\r" );
    return;
  }
  if( ( !str_cmp( argument, "t3" ) || !str_prefix( argument, "t3" ) ) && argument[0] != '\0' )
  {
    ch_printf( ch, "You can't.\n\r" );
    return;
  }
  if( ( !str_cmp( argument, "t4" ) || !str_prefix( argument, "t4" ) ) && argument[0] != '\0' )
  {
    ch_printf( ch, "You can't.\n\r" );
    return;
  }
  if( ( !str_cmp( argument, "omega" ) || !str_prefix( argument, "omega" ) ) && argument[0] != '\0' )
  {
    ch_printf( ch, "You can't.\n\r" );
    return;
  }
  if( ( !str_cmp( argument, "vm2" ) || !str_prefix( argument, "vm2" ) ) && argument[0] != '\0' )
  {
    ch_printf( ch, "You can't.\n\r" );
    return;
  }
  if( ( !str_cmp( argument, "vm1" ) || !str_prefix( argument, "vm1" ) ) && argument[0] != '\0' )
  {
    ch_printf( ch, "You can't.\n\r" );
    return;
  }

  if( ( !str_cmp( argument, "lssj" ) || !str_prefix( argument, "lssj" ) ) && argument[0] != '\0' )
  {
    ch_printf( ch, "You can't.\n\r" );
    return;
  }
  ...
  else
  {
    ...
    if( skill_table[sn]->min_level[ch->class] == 0
        ...
        || ( sn == gsn_super_dragonfist && ch->race == race_lookup( "saiyan" ) && !saiyanranked( ch ) )
        || ( sn == gsn_bbk && ch->race == race_lookup( "saiyan" ) && ch->rank < 2 ) )
    {
      act( AT_TELL, "$n tells you 'You can not learn that.'", mob, NULL, ch, TO_VICT );
      return;
    }
...
//  bool    canMystic = FALSE;
  bool canUseMystic = FALSE;
  bool trueMystic = FALSE;
  bool canUseLssj = FALSE;
  bool trueLssj = FALSE;
  bool canUseMajin = FALSE;
  bool trueMajin = FALSE;
  int ii = 0;
  ...
  for( sn = gsn_first_ability; sn < gsn_first_weapon; sn++ )
  {
    ...
    if( sn == gsn_wss && !demonranked( ch ) )
      continue;
    if( sn == gsn_bbk && ch->race == race_lookup( "saiyan" ) )
      if( ch->rank < 2 )
        continue;
    if( sn == gsn_super_dragonfist && ch->race == race_lookup( "saiyan" ) )
      if( !saiyanranked( ch ) )
        continue;
    ...
    /*
     * if( sn == gsn_sigma_virus && (ch->pcdata->learned[sn] < 1 || !is_android(ch)) )
     * continue;
     */
    if( ( sn == gsn_t4 || sn == gsn_t3 || sn == gsn_t2 ||
          sn == gsn_t1 || sn == gsn_omega || sn == gsn_vm1 ||
          sn == gsn_vm2 ) && ( ch->pcdata->learned[sn] < 1 || !is_android( ch ) ) )
      continue;

    if( sn == gsn_suppress && is_android( ch ) && ch->pcdata->learned[sn] < 1 )
      continue;

    if( sn == gsn_sent_chip && ( ch->pcdata->learned[sn] < 1 || !is_android( ch ) ) )
      continue;
    if( sn == gsn_super_oozaru && ( ch->pcdata->learned[sn] < 1 || !is_saiyan_s( ch ) ) )
      continue;
  ...
  for( i = 0; i < size; i++ )
  {
    ...
    if( !str_cmp( skill_table[snRace[i]]->name, "lssj" ) )
      trueLssj = TRUE;
    else
      trueLssj = FALSE;
    ...
    if( !str_cmp( skill_table[snRace[i]]->name, "majin" ) )
      trueMajin = TRUE;
    else
      trueMajin = FALSE;
    if( ch->canmajin )
    ...
    if( !canUse )
      if( !trueLssj )
        if( !trueMajin )
          if( trueMystic )
            if( canUseMystic )
              pager_printf_color( ch, "&z %22.22s       %16s   &Y%3.0f%%    &W%3d%%\n\r",
                                  skill_table[snRace[i]]->name,
                                  num_punct_ld( skill_table[snRace[i]]->skill_level[ch->class] ),
                                  ch->pcdata->learned[snRace[i]], skill_table[snRace[i]]->skill_adept[ch->class] );
...
void do_version( CHAR_DATA * ch, char *argument )
{
  ...
  set_char_color( AT_YELLOW, ch );
  ch_printf( ch, "&cDragonball: A New Age &w(DBNA) &Wv%s.%s.%s &w\n\r",
...
void do_upgrade( CHAR_DATA * ch, char *argument )
{
  ...
  if( ( !str_cmp( argument, "suppress" ) || !str_prefix( argument, "suppress" ) ) && argument[0] != '\0' )
  {
    ch_printf( ch, "You can't.\n\r" );
    return;
  }
  ...
  else
  {
    ...
    if( prac )
    {
      ...
      if( sn == gsn_t4 || sn == gsn_t3 || sn == gsn_t2 ||
          sn == gsn_t1 || sn == gsn_omega || sn == gsn_vm1 || sn == gsn_vm2 || sn == gsn_sent_chip || sn == gsn_suppress )
      {
