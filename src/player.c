 ...
 ****************************************************************************
 *			Dragonball: A New Age v1.5 Edition                  *
 ****************************************************************************/
...
void do_score( CHAR_DATA * ch, char *argument )
{
  ...
  pager_printf( ch, "\n\r&CThe %s,\n\r", get_rank( ch ) );
  pager_printf( ch, "%s%s%s.&C\n\r", ch->name, ch->pcdata->last_name ? ch->pcdata->last_name : "", ch->pcdata->title );
  ...
  pager_printf( ch, "RACE : &W%-16.16s&C    SEX: &W%-7s&C     Played: &w%d hours&C\n\r",
                ...
                ch->sex == SEX_MALE ? "Male" : ch->sex == SEX_FEMALE ? "Female" : "Neutral", ( get_age( ch ) - 4 ) );
  ...
  if( get_curr_dex( ch ) >= 100 || ch->perm_dex >= 100 )
    pager_printf( ch, "SPD  : &W%-3d&C(&w%3d.%2.2d&C)  pDEF: &R%-2d&C            Last Saved: &w%s\r&C",
                  get_curr_dex( ch ), ch->perm_dex, ch->pcdata->tSpd, ( get_strDef( ch ) + get_conDef( ch ) ),
  ...
  if( is_android( ch ) )
  {
    pager_printf_color( ch,
                        ...
                        ch->train, ch->hit, '%', xIS_SET( ch->act, PLR_AUTO_COMPASS ) ? 'X' : ' ', xIS_SET( ch->act,
                                                                                                            PLR_AUTOGOLD ) ?
                        'X' : ' ' );
  }
  else
  ...
  if( is_android( ch ) )
  {
    if( ch->max_mana < 1000 )
      pager_printf_color( ch,
                          ...
                          buf, buf2, xIS_SET( ch->act, PLR_AUTOEXIT ) ? 'X' : ' ', xIS_SET( ch->act,
                                                                                            PLR_AUTOLOOT ) ? 'X' : ' ' );
  ...
  if( is_android( ch ) && ch->pcdata->learned[gsn_self_destruct] > 0 )
    pager_printf_color( ch, "                                              Self Destruct Charges: [&R%5.5d&C]\n\r",
                        ch->pcdata->sd_charge );
  if( is_saibaman( ch ) && ch->pcdata->learned[gsn_jibaku] > 0 )
  ...
  if( ch->pcdata->clan )
  {
    ...
    pager_printf( ch, "&c Current Rank: (&w%d&c)&W%-20s  &cTotal Clan Members: (&w%d&c)\n\r", ch->pcdata->clanRank,
                  get_clanTitle( ch ), ch->pcdata->clan->members );
    pager_printf( ch, "&c Total Zeni Donated: (&W%s&c) Total Zeni Taxed: (&W%s&c)\n\r",
                  num_punct_d( ch->pcdata->clanZeniDonated ), num_punct_d( ch->pcdata->clanZeniClanTax ) );
    send_to_pager( "&c----------------------------------------------------------------------------&C\n\r", ch );
  }
  if( ch->pcdata->deity )
  {
...
char *get_saiyan( CHAR_DATA * ch )
{
  switch ( ch->rank )
  {
    ...
    case 2:
      if( ch->sex == SEX_MALE )
        return ( "Saiyan Prince" );
      if( ch->sex == SEX_FEMALE )
        return ( "Saiyan Princess" );
      if( ch->sex != SEX_FEMALE && ch->sex != SEX_MALE )
        return ( "Low Royal Saiyan" );
      break;
    case 3:
      if( ch->sex == SEX_MALE )
        return ( "Saiyan King" );
...
bool saiyanranked( CHAR_DATA * ch )
{
