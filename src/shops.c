...
CHAR_DATA *find_keeper( CHAR_DATA * ch )
{
  ...
  {
    do_say( keeper, "OMGZ0RIAMN0TSELLINJ00SHIT!!!! Pfft, immys... who needs 'em." );
    return NULL;
  }
...
/*      bool listall; */
    ...
    for( obj = keeper->first_carrying; obj; obj = obj->next_content )
    {
      ...
      {
// Pbase with coders! That's DBNA for ya! fixed and ajusted for Long Double
// code. THANKS, Geh! -Karn & Gehenom 04.01.05
        if( !found )
        {
          found = TRUE;
        ...
        num++;
        pager_printf( ch, "&W[%2d|%15s|", num, num_punct_ld( obj->level ) );
        pager_printf( ch, "%12s&w]&Y %s.\n\r", num_punct( cost ), obj->short_descr );
      }
    }
...
void do_sell( CHAR_DATA * ch, char *argument )
{
  ...
  sprintf( buf, "You sell $p for %d zeni.", cost );
                     ...
                     char *arg, int maxgold, char *fixstr, char *fixstr2 )
{
  ...
  {
    sprintf( buf, "$N tells you, 'It will cost %d zeni to %s %s...'", cost, fixstr, obj->name );
  ...
  else
  {
    ...
    sprintf( buf, "$N charges you %d zeni to %s $p.", cost, fixstr );
...
void appraise_all( CHAR_DATA * ch, CHAR_DATA * keeper, char *fixstr )
{
  ...
  {
    send_to_char( "\n\r", ch );
    sprintf( buf, "$N tells you, 'It will cost %d zeni in total.'", total );
    act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
...
void do_appraise( CHAR_DATA * ch, char *argument )
{
  ...
  sprintf( buf, "$N tells you, 'It will cost %d zeni to %s that...'", cost, fixstr );
  act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
  if( cost > ch->gold )
    act( AT_TELL, "$N tells you, 'Which I see you can't afford.'", ch, NULL, keeper, TO_CHAR );
