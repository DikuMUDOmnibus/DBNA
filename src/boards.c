...
void do_note( CHAR_DATA * ch, char *arg_passed, bool IS_MAIL )
{
  ...
  if( !str_cmp( arg, "list" ) )
  {
    ...
    else
    {
      ...
      for( pnote = board->first_note; pnote; pnote = pnote->next )
        if( is_note_to( ch, pnote ) || get_trust( ch ) >= sysdata.read_all_mail )
          ch_printf( ch, "%2d%c %s: %s\n\r", ++vnum, is_note_to( ch, pnote ) ? '-' : '}', pnote->sender, pnote->subject );
      return;
    }
  }
  ...
  if( !str_cmp( arg, "to" ) )
  {
    ...
    if( strchr( arg_passed, '@' ) != NULL )
    {
      ...
      {
        ch_printf( ch, "You need to be at least level %d to send " "notes to other muds.\n\r", sysdata.imc_mail_level );
        return;
      }
      imc = TRUE;
    }
  ...
  if( !str_cmp( arg, "post" ) )
  {
...
#ifdef USE_IMC
    if( ( imc && board->board_obj != sysdata.imc_mail_vnum ) || ( !imc && board->board_obj == sysdata.imc_mail_vnum ) )
    {
    ...
    if( imc )
      imc_post_mail( ch, pnote->sender, pnote->to_list, pnote->date, pnote->subject, pnote->text );
#endif
...
void do_bstat( CHAR_DATA * ch, char *argument )
{
  ...
  ch_printf_color( ch,
                   ...
                   board->note_file, board->board_obj, board->min_read_level, board->min_post_level, board->min_remove_level,
