...
static mchunkptr mmap_resize( mstate m, mchunkptr oldp, size_t nb )
{
   ...
   */
  if( oldsize >= nb + SIZE_T_SIZE && ( oldsize - nb ) <= 2U * mparams.granularity )
    return oldp;
  else
  {
    size_t offset = oldp->prev_foot & ~IS_MMAPPED_BIT;
    size_t oldmmsize = oldsize + offset + MMAP_FOOT_PAD;
    size_t newmmsize = granularity_align( nb + 6 * SIZE_T_SIZE + CHUNK_ALIGN_MASK );
  ...
  if( MORECORE_CONTIGUOUS && !use_noncontiguous( m ) )
  {
    ...
    { /* First time through or recovery */
      ...
      if( base != CMFAIL )
      {
         ...
         */
        if( asize < MAX_SIZE_T / 2 && ( brk = ( char * )( CALL_MORECORE( asize ) ) ) == base )
        {
          tbase = base;
          tsize = ( size_t ) asize;
        }
    ...
    else
    {
       ...
       */
      if( asize < MAX_SIZE_T / 2 && ( brk = ( char * )( CALL_MORECORE( asize ) ) ) == ss->base + ss->size )
      {
        tbase = brk;
        tsize = ( size_t ) asize;
      }
    ...
    if( tbase == CMFAIL )
    {
      ...
      if( brk != CMFAIL )
      { /* Try to use the space we did get */
  ...
  if( tbase != CMFAIL )
  {
    ...
    else
    {
      ...
      while( sp != 0 && tbase != sp->base + sp->size )
        sp = sp->next;
      if( sp != 0 && ( sp->sflags & IS_MMAPPED_BIT ) == mmap_flag && segment_holds( sp, m->top ) )
      { /* append */
        sp->size += tsize;
    ...
    if( HAVE_MMAP && use_noncontiguous( m ) )
    {
      ...
      while( sp != 0 )
      {
        ...
        if( ( sp->sflags & IS_MMAPPED_BIT ) )
        {
           ...
           */
          if( !cinuse( p ) &&
              p != m->top && segment_holds( sp, ( char * )pred ) && ( char * )p + psize >= base + size - TOP_FOOT_SIZE )
          {
  ...
  else
  {
    ...
    if( mem != 0 )
    {
      ...
      { /* misaligned */
         ...
         * possible.
         */
        char *brk = ( char * )mem2chunk( ( size_t ) ( ( ( size_t ) ( mem + alignment - 1 ) ) & -alignment ) );
        char *pos = ( ( size_t ) ( brk - ( char * )( p ) ) >= MIN_CHUNK_SIZE ) ? brk : brk + alignment;
  ...
  if( ok_magic( ms ) )
  {
    ...
    if( sp != 0 )
    {
      ...
      while( sp != 0 )
      {
        ...
        if( ( flag & IS_MMAPPED_BIT ) && CALL_MUNMAP( base, size ) == 0 )
          freed += size;
        sp = next;
      }
    }
     ...
     * free main space 
     */
    if( ( mflag & IS_MMAPPED_BIT ) && CALL_MUNMAP( ( char * )( mem2chunk( ms ) ), msize ) == 0 )
      freed += msize;
  }
  else
  {
