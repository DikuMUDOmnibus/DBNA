  ...
  else
  {
    ...
    if( !str_cmp( "Public", ship->owner ) || ship->type == MOB_SHIP )
    {
      ...
      ship->autospeed = FALSE;
             ...
             ship->cockpit == ROOM_EARTH_TURBOCAR || ship->cockpit == ROOM_CORUSCANT_SHUTTLE )
    {
    }
...
void update_traffic(  )
{
  ...
  if( turbocar != NULL )
//  && get_room_index(turbocar->location) != NULL) Blah blah blah --Saiyr
  {
    turbocar_delay++;
    if( turbocar_delay == 2 )
    {
      if( get_room_index( turbocar->location ) )
      {
...
void do_showship( CHAR_DATA * ch, char *argument )
{
  ...
  pager_printf_color( ch, "Firstroom: %d   Lastroom: %d", ship->firstroom, ship->lastroom );
...
void do_shipinfo( CHAR_DATA * ch, char *argument )
{
  ...
  pager_printf_color( ch, "Laser cannons: %d  ", target->lasers );
  pager_printf_color( ch, "Maximum Missiles: %d  ", target->maxmissiles );
  pager_printf_color( ch, "Max Chaff: %d\n\r", target->maxchaff );
  pager_printf_color( ch, "Max Hull: %d  ", target->maxhull );
...
*/
  ...
  pager_printf_color( ch, "&OLaser Condition:&Y %s  &OCurrent Target:&Y %s\n\r",
                      target->statet0 == LASER_DAMAGED ? "Damaged" : "Good",
  ...
  pager_printf_color( ch, "\n\r&OMissiles:&Y %d&O/%d  Torpedos: &Y%d&O/%d  Rockets: &Y%d&O/%d  Condition:&Y %s&w\n\r",
                      ship->missiles, ship->maxmissiles, ship->torpedos, ship->maxtorpedos, ship->rockets, ship->maxrockets,
...
*/
      if( ship->shipstate == SHIP_HYPERSPACE && !str_cmp( argument, "drive" ) )
      {
    ...
    if( ch->level < to_room->area->low_hard_range )
    {
      ...
      switch ( to_room->area->low_hard_range - ch->level )
      {
        case 1:
          ...
          break;
        case 2:
          send_to_pager_color( "A voice in your mind says, 'Soon you shall be ready to travel down this path... soon.'",
                               ch );
          break;
        case 3:
...
bool is_dock( ROOM_INDEX_DATA * room )
{
  ...
  for( starsystem = first_starsystem; starsystem; starsystem = starsystem->next )
  {
    if( vnum == starsystem->doc1a || vnum == starsystem->doc1b
        ...
        || vnum == starsystem->doc3a || vnum == starsystem->doc3b || vnum == starsystem->doc3c )
      return TRUE;
  }
  return FALSE;
}
