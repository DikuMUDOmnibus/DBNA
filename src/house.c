...
void do_house( CHAR_DATA * ch, char *argument )
{
  ...
  if( !str_cmp( arg, "set" ) )
  {
    ...
    if( !arg2 || arg2[0] == '\0' || !arg3 || arg3[0] == '\0' || ( !is_number( arg3 ) && str_cmp( arg3, "addroom" ) ) )
    {
...
void do_gohome( CHAR_DATA * ch, char *argument )
{
  ...
  sprintf( buf, "As a vortex of luminescent light forms, %s slides gracefully through into their private sanctuary.",
           ch->name );
  act( AT_GREY, buf, ch, NULL, ch, TO_ROOM );
    ...
    else
    {
      ...
      ch_printf_color( ch, "&C&gAverage Hp: &w%d\n\r",
                       ( !mob->hitnodice ? ( mob->level * 8 + number_range( mob->level * mob->level / 4,
                                                                            mob->level * mob->level ) ) : ( mob->hitnodice *
                                                                                                            number_range( 1,
                                                                                                                          mob->
                                                                                                                          hitsizedice )
                                                                                                            +
                                                                                                            mob->
                                                                                                            hitplus ) ) );
      ch_printf_color( ch, "&C&gAverage AC: &w%d\n\r", ( mob->ac ? mob->ac : interpolate( mob->level, 100, -100 ) ) );
    ...
    if( !str_cmp( arg, "list" ) )
    {
      ...
      for( tmpres = first_homebuy; tmpres; tmpres = tmpres->next )
      {
        ...
        ch_printf_color( ch, "&C&c%2d &C| &c%14s &C| &c%12s &C| &c%11s &C| &c%10s &C| &c%2d &CDays &c%2d &CHrs &C|\n\r",
                         i, name, area, buf2, buf3, days, ( ( tmpres->endtime - ( days * 48 ) ) / 2 ) );
