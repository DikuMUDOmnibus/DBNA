...
bool can_see_obj( CHAR_DATA * ch, OBJ_DATA * obj )
{
  ...
  if( room_is_dark( ch->in_room ) )
  {
    ...
    if( IS_OBJ_STAT( obj, ITEM_GLOW ) )
      return TRUE;
    if( !IS_AFFECTED( ch, AFF_INFRARED ) && !is_android( ch ) && !is_superandroid( ch ) && !is_demon( ch ) )
      return FALSE;
  }
...
bool can_astral( CHAR_DATA * ch, CHAR_DATA * victim )
{
  if( victim == ch
      ...
      || ( get_attmod( ch, victim ) > 10 && ( ch->rank < 1 ) && !wearing_sentient_chip( ch ) )
