...
void do_wff( CHAR_DATA * ch, char *argument )
{
  ...
  {
    act( AT_SKILL, "&rCrouching down with your right leg in front, you retain all your focus on", ch, NULL, victim, TO_CHAR );
    ...
    act( AT_SKILL, "&r$N. Without warning, $n bursts into a charge and calls out &R'Wolf Fang", ch, NULL, victim, TO_NOTVICT );
    act( AT_SKILL, "&RFist'. &rAn impression of a savage wolf emerges from $s clasping palms", ch, NULL, victim, TO_NOTVICT );
...
void do_vanishing_ball(CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->exp < skill_table[gsn_vanishing]->skill_level[ch->class] )
    {
	send_to_char( "You can't do that.\n\r", ch );
	return;
    }
	...
	if (ch->charge > 0)
		dam = chargeDamMult(ch, dam);
	act( AT_LBLUE, "A large ball of black and blue, swirling, energy begins to form in your hands. With an evil grin on your face you yells out 'Super Vanishing Spirit Ball'! suddenly this omnious ball filled with soul energy from both the good and evil intentions blasts toward $N hitting them with such force it goes directly into their body and explodes from the inside ripping their soul appart from the inside. &W[$t]", ch, num_punct(dam), victim, TO_CHAR );
	act( AT_LBLUE, "$n forms a large black and blue, swirling, energy ball into $s hands. With an evil grin on their face they yell 'Super Vanishing Spirit Ball'! The ball flys tword you with such intensity it feels like it went inside you. After several seconds you feel your soul being ripped appart from inside your own being. &W[$t]", ch, num_punct(dam), victim, TO_VICT );
	act( AT_LBLUE, "$n forms a large black and blue, swirling, energy ball into $s hands.With an evil grin on their face they yell 'Super Vanishing Spirit Ball'! The ball flys tword $N with such intensity it looks like it went inside of them. After several seconds $N begins to scream as if their very soul were being ripped out. &W[$t]", ch, num_punct(dam), victim, TO_NOTVICT );
