...
void do_mpstat( CHAR_DATA * ch, char *argument )
{
  ...
  ch_printf( ch,
             ...
             victim->level, victim->class, victim->alignment, GET_AC( victim ), victim->gold, num_punct_ld( victim->exp ) );
 ...
 *
 * lol. Silly Warren. -Karn 02.23.05
 */

      gain_exp( victim, ( ( victim->exp * 0.0009 ) * -11 ) );
...
void do_mpshowbot( CHAR_DATA * ch, char *argument )
{
  ...
  pager_printf( victim,
                "&C%s says, 'Please say the characters located at A-%d, B-%d, C-%d, D-%d and E-%d back to me.'&D\n\r",
