...
//    pager_printf_color( ch, "&cPlayerFlags: &w%s\n\r", ext_flag_string( &victim->act, plr_flags ) );
  ...
  if( is_kaio( victim ) )
  {
    ch_printf( ch, "&cKai rank: &w%d &c(&w%s&c)\n\r", victim->rank, get_kai( victim ) );
  }
  if( is_demon( victim ) )
  {
    ch_printf( ch, "&cDemon rank: &w%d &c(&w%s&c)\n\r", victim->rank, get_demon( victim ) );
  }
  if( is_saiyan( victim ) )
  {
    ch_printf( ch, "&cSaiyan rank: &w%d &c(&w%s&c)\n\r", victim->rank, get_saiyan( victim ) );
  }
  ch_printf( ch, "\n\r" );
  return;
}
...
void do_shutdown( CHAR_DATA * ch, char *argument )
{
  ...
  if( str_cmp( argument, "this mud now" ) && str_cmp( argument, "this nosave" ) )
  {
    send_to_char( "Syntax:  'shutdown <pwd> mud now' or 'shutdown <pwd> nosave'\n\r", ch );
    return;
  }
...
void do_oinvoke( CHAR_DATA * ch, char *argument )
{
  ...
  if( ( pObjIndex->item_type == ITEM_DRAGONBALL || pObjIndex->item_type == ITEM_DRAGONRADAR ) ) //&& ch->level < 62 )
  {
