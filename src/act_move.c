...
ch_ret move_char( CHAR_DATA * ch, EXIT_DATA * pexit, int fall )
{
  ...
  if( !pexit || !to_room )
  {
    if( drunk && ch->position != POS_MOUNTED
        ...
        && ch->in_room->sector_type != SECT_OCEANFLOOR && ch->in_room->sector_type != SECT_SPACE )
    {
      switch ( number_bits( 4 ) )
      {
        default:
          ...
          break;
        case 3:
          act( AT_ACTION, "In your drunken stupor you trip over your own feet and tumble to the ground.", ch, NULL, NULL,
               TO_CHAR );
  ...
  if( IS_SET( pexit->exit_info, EX_NOMOB ) && IS_NPC( ch ) && !is_split( ch ) )
  {
  ...
  {
    send_to_char( "The person in there is not designated as your partner " "for the Time Chamber.\n\r", ch );
    return rNONE;
  }
  ...
  if( !fall && !IS_NPC( ch ) )
  {
    ...
    if( in_room->sector_type == SECT_AIR
        ...
        || to_room->sector_type == SECT_AIR || to_room->sector_type == SECT_SPACE || IS_SET( pexit->exit_info, EX_FLY ) )
    {
