...
int demonkinglast;
int saiyanelitelast[4];
int saiyanprincelast[2];
int saiyankinglast;
...
void untransform_oozaru( CHAR_DATA * ch )
{
  ...
  act( AT_WHITE, "effects the transformation has brought about. In a timely fashion, $s monstrous", ch, NULL, NULL, TO_NOTVICT );
...
/*  PL high enough to gain more energy */
  ...
  if( ch->exp >= ( pow( ch->max_energy, 2.27 ) * 750 ) && ch->max_mana < 99999 )
  {
    set_char_color( AT_LBLUE + AT_BLINK, ch );
...
void mana_check( void )
{
  ...
  for( ch = first_char; ch; ch = ch->next )
  {
    ...
    if( is_super_saiyan( ch ) && ch->mana <= minMANA && ch->exp < ch->pl )
      act( AT_WHITE, "Your energy has become to weak to sustain your transformation.", ch, NULL, NULL, TO_CHAR );
    if( is_super_saiyan( ch ) && ch->mana <= minMANA && ch->exp < ch->pl )
      do_powerdown( ch, ch->name );
    continue;
/*
	if (xIS_SET(ch->affected_by, AFF_SNAMEK))
	  ch_printf(ch,"Super Saiyan, Pulse. Beat that meat!\n\r");
	...
	if (xIS_SET(ch->affected_by, AFF_KAIOKEN))
	  ch_printf(ch,"Super Saiyan, Pulse. Beat that meat!\n\r");
*/
    if( xIS_SET( ch->affected_by, AFF_SSJ ) && is_fighting )
      ch->mana -= 100;
//  if (xIS_SET(ch->affected_by, AFF_SSJ) && is_fighting)
//          send_to_char("\n\r",ch);
    if( xIS_SET( ch->affected_by, AFF_SSJ2 ) && is_fighting )
      ch->mana -= 100;
    if( xIS_SET( ch->affected_by, AFF_SSJ3 ) && is_fighting )
      ch->mana -= 100;
    if( xIS_SET( ch->affected_by, AFF_SSJ4 ) && is_fighting )
      ch->mana -= 100;
    if( xIS_SET( ch->affected_by, AFF_USSJ ) && is_fighting )
      ch->mana -= 50;
    if( xIS_SET( ch->affected_by, AFF_USSJ2 ) && is_fighting )
      ch->mana -= 100;
...
*/
    ...
    if( get_trust( ch ) > 65 )
      send_to_char( "Your level is to High. Please log off now.", ch );
    else
      continue;
  }
}
...
void rank_update( void )
{
  ...
  for( a = 0; a < 6; a++ )
  {
    if( str_cmp( kaioshin[a], "none" ) )
    {
      ...
      if( ( ch = get_char_name( kaioshin[a] ) ) )
      {
        if( !is_kaio( ch ) )
        {
          bug( "rank_update: ch is not a kaioshin (%s)", ch->name );
          ch->rank = 0;
          STRFREE( kaioshin[a] );
      ...
      /*
       * If the kaio bails for more than 3 days, strip their rank. 
       * * -Karma
       */
      if( ( ( current_time - kaioshinlast[a] ) / 86400 ) >= 3 )
      {
        sprintf( buf, "&R%s has lost their kaioshin rank due to inactivity.\n\r", capitalize( kaioshin[a] ) );
        STRFREE( kaioshin[a] );
        kaioshin[a] = STRALLOC( "none" );
        kaioshinlast[a] = current_time;
        save_sysdata( sysdata );
  ...
  for( a = 0; a < 6; a++ )
  {
    if( str_cmp( greaterdemon[a], "none" ) )
    {
      ...
      if( ( ch = get_char_name( greaterdemon[a] ) ) )
      {
        if( !is_demon( ch ) )
        {
          bug( "rank_update: ch is not a demon (%s)", ch->name );
          ch->rank = 0;
      ...
      /*
       * If the demon bails for more than 3 days, strip their rank.
       * * -Karma
       */
      if( ( ( current_time - greaterdemonlast[a] ) / 86400 ) >= 3 )
      {
        sprintf( buf, "&R%s has lost their demon rank due to inactivity.\n\r", capitalize( greaterdemon[a] ) );
        STRFREE( greaterdemon[a] );
        greaterdemon[a] = STRALLOC( "none" );
        greaterdemonlast[a] = current_time;
        save_sysdata( sysdata );
  ...
  for( a = 0; a < 3; a++ )
  {
    if( str_cmp( demonwarlord[a], "none" ) )
    {
      ...
      if( ( ch = get_char_name( demonwarlord[a] ) ) )
      {
        if( !is_demon( ch ) )
        {
          bug( "rank_update: ch is not a demon (%s)", ch->name );
       ...
       * * -Karma
       */
      if( ( ( current_time - demonwarlordlast[a] ) / 86400 ) >= 3 )
      {
        sprintf( buf, "&R%s has lost their demon rank due to inactivity.\n\r", capitalize( demonwarlord[a] ) );
        STRFREE( demonwarlord[a] );
        demonwarlord[a] = STRALLOC( "none" );
        demonwarlordlast[a] = current_time;
        save_sysdata( sysdata );
  ...
  if( str_cmp( demonking, "none" ) )
  {
     ...
     * * -Karma
     */
    if( ( ( current_time - demonkinglast ) / 86400 ) >= 3 )
    {
      sprintf( buf, "&R%s has lost their demon rank due to inactivity.\n\r", capitalize( demonking ) );
      STRFREE( demonking );
      demonking = STRALLOC( "none" );
      demonkinglast = current_time;
      save_sysdata( sysdata );
  ...
  for( a = 0; a < 4; a++ )
  {
    if( str_cmp( saiyanelite[a], "none" ) )
    {
      ...
      if( ( ch = get_char_name( saiyanelite[a] ) ) )
      {
        if( !is_saiyan( ch ) )
        {
          bug( "rank_update: ch is not a saiyan (%s)", ch->name );
          ch->rank = 0;
      ...
      /*
       * If the Saiyan bails for more than 3 days, strip their rank.
       */
      if( ( ( current_time - saiyanelitelast[a] ) / 86400 ) >= 3 )
      {
        sprintf( buf, "&R%s has lost their saiyan rank due to inactivity.\n\r", capitalize( saiyanelite[a] ) );
        STRFREE( saiyanelite[a] );
        saiyanelite[a] = STRALLOC( "none" );
        saiyanelitelast[a] = current_time;
        save_sysdata( sysdata );
  ...
  for( a = 0; a < 2; a++ )
  {
    if( str_cmp( saiyanprince[a], "none" ) )
    {
      ...
      if( ( ch = get_char_name( saiyanprince[a] ) ) )
      {
        if( !is_saiyan( ch ) )
        {
          bug( "rank_update: ch is not a saiyan (%s)", ch->name );
       ...
       * If the Saiyan bails for more than 3 days, strip their rank.
       */
      if( ( ( current_time - saiyanprincelast[a] ) / 86400 ) >= 3 )
      {
        sprintf( buf, "&R%s has lost their saiyan rank due to inactivity.\n\r", capitalize( saiyanprince[a] ) );
        STRFREE( saiyanprince[a] );
        saiyanprince[a] = STRALLOC( "none" );
        saiyanprincelast[a] = current_time;
        save_sysdata( sysdata );
  ...
  if( str_cmp( saiyanking, "none" ) )
  {
    ...
    if( ( ch = get_char_name( saiyanking ) ) )
    {
      if( !is_saiyan( ch ) )
      {
        bug( "rank_update: ch is not a saiyan (%s)", ch->name );
     ...
     * If the saiyan bails for more than 3 days, strip their rank.
     */
    if( ( ( current_time - saiyankinglast ) / 86400 ) >= 3 )
    {
      sprintf( buf, "&R%s has lost their saiyan rank due to inactivity.\n\r", capitalize( saiyanking ) );
      STRFREE( saiyanking );
      saiyanking = STRALLOC( "none" );
      saiyankinglast = current_time;
      if( ch )
...
*/
    ...
    if( number_percent(  ) < 25 && !IS_IMMORTAL( ch ) )
    {
       ...
       if( IS_AFFECTED( ch, AFF_LSSJ ) )
      {
        ...
        act( AT_YELLOW, "Your body shifts abit as your aura flaming with legendary power", ch, NULL, NULL, TO_CHAR );
        act( AT_YELLOW, "$n's body shifts abit as $s aura flaming with legendary power", ch, NULL, NULL, TO_NOTVICT );
      }
