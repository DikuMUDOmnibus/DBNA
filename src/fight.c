...
bool pkill_ok( CHAR_DATA * ch, CHAR_DATA * victim )
{
  ...
  {
    if( is_kaio( ch ) && kairanked( victim ) && ch->rank <= victim->rank )
      clanbypass = TRUE;
      ...
      clanbypass = TRUE;
    else if( is_saiyan( ch ) && saiyanranked( victim ) )
      clanbypass = TRUE;
    else if( saiyanranked( ch ) && saiyanranked( victim ) )
      clanbypass = TRUE;
  ...
  if( is_saiyan( ch ) && saiyanranked( victim ) )
  {
    return TRUE;
  }
  ...
  if( saiyanranked( ch ) && kairanked( victim ) )
  {
    if( ( float )ch->exp / victim->exp > 5 && !IS_HC( victim ) )
    {
      ch_printf( ch, "You are 5 times stronger.\n\r" );
  ...
  if( saiyanranked( ch ) && demonranked( victim ) )
  {
    if( ( float )ch->exp / victim->exp > 5 && !IS_HC( victim ) )
    {
      ch_printf( ch, "You are 5 times stronger.\n\r" );
  ...
  if( kairanked( ch ) && saiyanranked( victim ) )
  {
    if( ( float )ch->exp / victim->exp > 5 && !IS_HC( victim ) )
    {
      ch_printf( ch, "You are 5 times stronger.\n\r" );
  ...
  if( demonranked( ch ) && saiyanranked( victim ) )
  {
    if( ( float )ch->exp / victim->exp > 5 && !IS_HC( victim ) )
    {
      ch_printf( ch, "You are 5 times stronger.\n\r" );
...
//      && number_range(1, 100) < 35)
  ...
  if( victim->position == POS_DEAD )
  {
    ...
    if( !IS_NPC( ch ) && !IS_NPC( victim ) )
    {
      if( is_kaio( ch ) && kairanked( victim ) )
      {
        if( ch->rank < victim->rank )
        {
          int lower = 0;
          int higher = 0;
          sprintf( kbuf, "%s has stolen %s's kaioshin title %s", ch->name, victim->name, get_kai( victim ) );
          to_channel( kbuf, CHANNEL_MONITOR, "Monitor", LEVEL_IMMORTAL );
          sprintf( kbuf, "%s has stolen %s's kaioshin title of %s", ch->name, victim->name, get_kai( victim ) );
          do_info( ch, kbuf );
          lower = ch->rank;
          higher = victim->rank;
          ...
          if( victim->rank == 0 )
          {
            sprintf( kbuf, "%s is a weak Kaioshin", victim->name );
            do_info( ch, kbuf );
          ...
          {
            sprintf( kbuf, "%s has dropped to kaioshin title %s", victim->name, get_kai( victim ) );
      ...
      if( is_demon( ch ) && demonranked( victim ) )
      {
        if( ch->rank < victim->rank )
        {
          ...
          sprintf( kbuf, "%s has stolen %s's demon title %s", ch->name, victim->name, get_demon( victim ) );
          to_channel( kbuf, CHANNEL_MONITOR, "Monitor", LEVEL_IMMORTAL );
          sprintf( kbuf, "%s has stolen %s's demon title of %s", ch->name, victim->name, get_demon( victim ) );
          do_info( ch, kbuf );
          lower = ch->rank;
          higher = victim->rank;
          if( vg )
          ...
          if( victim->rank == 0 )
          {
            sprintf( kbuf, "%s is a weak Demon.", victim->name );
            do_info( ch, kbuf );
          }
          else
          {
            sprintf( kbuf, "%s has dropped to demon title %s", victim->name, get_demon( victim ) );
            if( cg )
            {
      ...
      if( is_pure_saiyan( ch ) && saiyanranked( victim ) )
      {
        if( ch->rank < victim->rank )
        {
          int lower = 0;
          int higher = 0;
          bool ce = FALSE, ve = FALSE;  // saiyan elite
          bool cp = FALSE, vp = FALSE;  // saiyan prince
          bool vk = FALSE;  // saiyan king
          int c = 0;
          int d = 0;
          int e = 0;
          if( saiyanranked( ch ) )
          {
            if( ch->rank == 1 )
            {
              for( c = 0; c < 4; c++ )
              {
                if( !str_cmp( saiyanelite[c], ch->name ) )
                {
                  ce = TRUE;
                  d = c;
                  break;
            ...
            else if( ch->rank == 2 )
            {
              for( c = 0; c < 2; c++ )
              {
                if( !str_cmp( saiyanprince[c], ch->name ) )
                {
                  cp = TRUE;
                  d = c;
                  break;
          ...
          if( saiyanranked( victim ) )
          {
            if( victim->rank == 1 )
            {
              for( c = 0; c < 4; c++ )
              {
                if( !str_cmp( saiyanelite[c], victim->name ) )
                {
                  ve = TRUE;
                  e = c;
                  break;
            ...
            else if( victim->rank == 2 )
            {
              for( c = 0; c < 2; c++ )
              {
                if( !str_cmp( saiyanprince[c], victim->name ) )
                {
                  vp = TRUE;
                  e = c;
                  break;
          ...
          sprintf( kbuf, "%s has stolen %s's saiyan title %s", ch->name, victim->name, get_saiyan( victim ) );
          to_channel( kbuf, CHANNEL_MONITOR, "Monitor", LEVEL_IMMORTAL );
          sprintf( kbuf, "%s has stolen %s's saiyan title of %s", ch->name, victim->name, get_saiyan( victim ) );
          do_info( ch, kbuf );
          lower = ch->rank;
          higher = victim->rank;
          if( ve )
          {
            STRFREE( saiyanelite[e] );
            saiyanelite[e] = STRALLOC( ch->name );
          }
          else if( vp )
          {
            STRFREE( saiyanprince[e] );
            saiyanprince[e] = STRALLOC( ch->name );
          }
          else if( vk )
          {
            STRFREE( saiyanking );
            ...
            if( xIS_SET( victim->affected_by, AFF_SANCTUARY ) )
              xREMOVE_BIT( victim->affected_by, AFF_SANCTUARY );
            victim->pcdata->learned[gsn_lssj] = 0;
            REMOVE_BIT( victim->pcdata->flags, PCFLAG_KNOWSLSSJ );
          }
          ch->rank = higher;
          victim->rank = lower;
          if( ch->rank == 3 )
          ...
          if( victim->rank == 0 )
          {
            sprintf( kbuf, "%s is a weak Saiyan.", victim->name );
            do_info( ch, kbuf );
          }
          else
          {
            sprintf( kbuf, "%s has dropped to saiyan title %s", victim->name, get_saiyan( victim ) );
            if( ce )
            {
              STRFREE( saiyanelite[d] );
              saiyanelite[d] = STRALLOC( victim->name );
            }
            else if( cp )
            {
              STRFREE( saiyanprince[d] );
              saiyanprince[d] = STRALLOC( victim->name );
            }
            do_info( ch, kbuf );
          }
          save_sysdata( sysdata );
    ...
    if( !npcvict )
    {
      ...
      sprintf( buf, "%s has been &Rkilled&B by %s", victim->name, ( IS_NPC( ch ) ? ch->short_descr : ch->name ) );
      do_info( ch, buf );
          ...
          if( ( preservation && !immortal ) || ( biopres && !immortal ) )
          {
            if( warpres || immortal )
              los = ( long double )victim->exp * 0.005;
                     ...
                     ( saiyanranked( ch ) && kairanked( victim ) ) ||
                     ( saiyanranked( ch ) && demonranked( victim ) ) ||
                     ( demonranked( ch ) && saiyanranked( victim ) ) ||
                     ( kairanked( ch ) && saiyanranked( victim ) ) || kaiopres || demonpres || saiyanpres )
...
bool is_safe( CHAR_DATA * ch, CHAR_DATA * victim, bool show_messg )
{
  ...
  if (( kairanked( victim ) || demonranked( victim ) 
    ||  saiyanranked( victim )) && is_fighting )
	return FALSE;
