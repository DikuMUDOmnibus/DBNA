...
GLOBAL_BOARD_DATA boards[MAX_BOARD] = {
  {"Announce", "&RAnnouncements from Admins", 0, LEVEL_IMMORTAL, "all", DEF_NORMAL, 60, NULL, FALSE, 1}
  ,
  ...
  ,
  {"RP", "[IC] Discussion", 0, 1, "all", DEF_INCLUDE, 14, NULL, FALSE, 1}
  ,
  {"Ideas", "Suggestion for improvement", 0, 1, "all", DEF_INCLUDE, 28, NULL, FALSE, 1}
  ,
  ...
  ,
  {"Quests", "Quest Board", 0, 1, "all", DEF_INCLUDE, 14, NULL, FALSE, 1}
  ,
};
...
void load_board( GLOBAL_BOARD_DATA * board )
{
  ...
  for( ;; )
  {
    ...
    {
//   Remove writing of the *.old note files
...
void do_nwrite( CHAR_DATA * ch, char *argument )
{
  ...
  if( ( ch->pcdata->board->write_level > get_trust( ch ) )
      || ( !ch->pcdata->clan && ch->pcdata->board->access == BOARD_CLAN_ONLY && !IS_IMMORTAL( ch ) )
      || ( !xIS_SET( ch->act, PLR_CAN_CHAT ) && !IS_IMMORTAL( ch ) && ch->pcdata->board->access == BOARD_AUTHED_BIO ) )
  {
...
void do_ncatchup( CHAR_DATA * ch, char *argument )
{
  ...
  else if( board_number( ch->pcdata->board ) == 0 )
  {
    send_to_char_color( "Announcements are important, you can't skip them.\n\r", ch );
    return;
  }
  else
  {
...
void do_global_boards( CHAR_DATA * ch, char *argument )
{
  ...
  if( !argument[0] )  /* show boards */
  {
    ...
    if( IS_IMMORTAL( ch ) )
    {
      send_to_char_color( "&WNum         &zName &WUnread &zPersonal &WClan &zImmort &WDescription&w\n\r"
                          "&w=== ============ ====== ======== ==== ====== ===========&w\n\r", ch );
    }
    else
    {
      send_to_char_color( "&WNum         &zName &WUnread &zPersonal &WClan &zDescription&w\n\r"
                          "&w=== ============ ====== ======== ==== ===========&w\n\r", ch );
    }
    for( i = 0; i < MAX_BOARD; i++ )
    {
      ...
      if( unread != BOARD_NOACCESS )
      {
        if( IS_IMMORTAL( ch ) )
        {
          sprintf( buf, "&W%2d&w> &z%12s&w [%s%4d&w] [%s%6d&w] [%s%2d&w] [%s%4d&w] &Y%s&w\n\r",
                   count, boards[i].short_name,
                   unread ? "&r" : "&W", unread,
                   unread_pers ? "&r" : "&W", unread_pers,
                   unread_clan ? "&r" : "&W", unread_clan, unread_imm ? "&r" : "&W", unread_imm, boards[i].long_name );
        }
        else
        {
          sprintf( buf, "&W%2d&w> &z%12s&w [%s%4d&w] [%s%6d&w] [%s%2d&w] &Y%s&w\n\r",
                   count, boards[i].short_name,
                   unread ? "&r" : "&W", unread,
                   unread_pers ? "&r" : "&W", unread_pers, unread_clan ? "&r" : "&W", unread_clan, boards[i].long_name );
        }
        send_to_char_color( buf, ch );
        count++;
      } /* if has access */
    ...
    if( ch->pcdata->board->read_level > get_trust( ch ) )
      send_to_char_color( "You cannot read nor write notes on this board.\n\r", ch );
             ...
             || ( !ch->pcdata->clan && ch->pcdata->board->access == BOARD_CLAN_ONLY && !IS_IMMORTAL( ch ) )
             || ( !xIS_SET( ch->act, PLR_CAN_CHAT ) && !IS_IMMORTAL( ch )
                  && ch->pcdata->board->access == BOARD_AUTHED_BIO ) )
  ...
  if( is_number( argument ) )
  {
    ...
    if( count == number ) /* found the board.. change to it */
    {
      ...
      send_to_char_color( buf, ch );
      if( ( get_trust( ch ) < boards[i].write_level )
          || ( !ch->pcdata->clan && boards[i].access == BOARD_CLAN_ONLY && !IS_IMMORTAL( ch ) )
          || ( !xIS_SET( ch->act, PLR_CAN_CHAT ) && !IS_IMMORTAL( ch ) && boards[i].access == BOARD_AUTHED_BIO ) )
